...
class WarmStartStrategy(BaseStrategy):
    """Implements the simple strategy"""

    def __init__(self, *args, interval: float = 1.0, keepalive_time: float = 300.0):
        """Initialize the flowcontrol object.

        We start the timer thread here

        :param interval: seconds after which timer expires
        :param keepalive_time: time that the manager and workers will stay alive
            for new executions after which strategy will try to kill them.
            Default: 300s (5min)
        """
        log.info("WarmStartStrategy Initialized")
    ...
    def _strategize(self, *args, **kwargs):
        ...
        if active_tasks == 0:
            ...
            # More blocks than min_blocks. Scale down
                ...
                # We want to make sure that keepalive_time was respected
                ...
                if _now - idle_since > self.keepalive_time:
                    # We have resources idle for the max duration,
                    # we have to scale_in now.
                    log.debug(
                        "Keep-alive time has reached {}s; removing resources".format(
                            self.keepalive_time
                        )
                    )
