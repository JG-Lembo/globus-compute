"""ColdStart builds on Parsl's HighThroughputExecutor for execution
of functions within containerized workers in a distributed setting,
allowing for warm starts of workers.
"""
...
from globus_compute_endpoint.engines.warm_start import interchange, zmq_pipes
from globus_compute_endpoint.engines.warm_start.mac_safe_queue import mpQueue
...
class WarmStartEngine(GlobusComputeEngineBase, RepresentationMixin):
    """Engine designed for cluster-scale

    The WarmStartEngine system has the following components:
      1. The WarmStartEngine instance which is run as a client
      ...
      4. ZeroMQ pipes connect the WarmStartEngine, Interchange and the
         process_worker_pool
    ...
    def __init__(
        ...
        log.debug("Initializing WarmStartEngine")
    ...
    def start(
        ...
        **kwargs,
        ...
        assert run_dir, "WarmStartEngine requires kwarg:run_dir at start"
        assert endpoint_id, "WarmStartEngine requires kwarg:endpoint_id at start"
        self.run_dir = run_dir
        self.endpoint_id = endpoint_id
        ...
        if self.provider:
            ...
            log.debug("Starting WarmStartEngine with no provider")
    ...
    def shutdown(self, /, hub=True, targets="all", block=False, **kwargs) -> None:
        ...
        log.info("Attempting WarmStartEngine shutdown")
        if self.queue_proc:
            try:
            ...
            log.info("Finished WarmStartEngine shutdown attempt")
...
class HTEXFuture(concurrent.futures.Future):
    ...
    def __init__(self, engine: WarmStartEngine, task_id: str):
        super().__init__()
        self.engine = engine
