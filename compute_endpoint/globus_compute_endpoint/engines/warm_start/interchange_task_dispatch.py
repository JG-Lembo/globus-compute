...
def assign_task_to_manager(
    ...
    interesting_managers: set[bytes],
    tids_by_manager: dict[bytes, dict[str, set[str]]],
    try_unused: bool = False,
) -> bool:

    # check unused workers if try_unused is True
    worker_type = "unused" if try_unused else task_type
    ...
    if not (real_capacity > 0 and mdata["active"]):
        ...
        return False
    if (
        worker_type in manager_free_cap["free"]
        and manager_free_cap["free"][worker_type] > 0
    ):
        if manager not in task_dispatch:
            task_dispatch[manager] = []
        ...
        tids_by_manager[manager][worker_type].add(task["task_id"])
        mdata["tasks"][task_type].add(task["task_id"])
        mdata["total_tasks"] += 1
        mdata["free_capacity"]["free"][worker_type] -= 1
